...
export default function POSPage() {
  ...
  return (
    <AppShell title="Restro POS">
      ...
      <nav className="lg:hidden sticky top-0 z-50 flex items-center gap-2 overflow-x-auto border-b border-slate-200 bg-white px-3 md:px-4 py-3 shadow-md">
        {internalNav.map((item) => {
          ...
          return (
            <NavLink
              ...
              to={item.path}
              className={({ isActive }) =>
                `flex flex-col items-center gap-1 rounded-2xl px-3 md:px-4 py-2 md:py-3 text-center whitespace-nowrap transition flex-shrink-0 border-2 ${
                  isActive
            ...
            >
              ...
              <span className="text-[10px] md:text-xs uppercase tracking-widest font-semibold">{item.label}</span>
            </NavLink>
          );
        })}
      </nav>
      ...
      <div className="mx-auto max-w-[1700px] px-4 py-5 sm:px-6 lg:px-8">
        <div className="grid min-h-[calc(100vh-160px)] grid-cols-1 lg:grid-cols-[minmax(0,1fr)_420px] gap-4 md:gap-6 bg-slate-50 px-3 md:px-4 py-3 md:py-4 rounded-[32px] shadow-[0_20px_80px_rgba(5,9,63,0.08)]">
          ...
          <main className="flex flex-col gap-4 md:gap-6 lg:col-span-1">

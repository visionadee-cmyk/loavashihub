...
export default function OutsourceItemsPage() {
  ...
  const beginBulkPayForSelected = (ids: string[]) => {
    const selected = items.filter((it) => ids.includes(it.id) && !it.partyPaid);
    if (selected.length === 0) return;
    setBulkItems(selected);
    const sel: Record<string, boolean> = {};
    selected.forEach((it) => { sel[it.id] = true; });
    setBulkSelectedIds(sel);
    setBulkPaymentDate(new Date().toISOString().slice(0, 10));
    setBulkDeductionDate(new Date().toISOString().slice(0, 10));
    setBulkPaymentSource('dailyRevenue');
  ...
  const selectedCount = useMemo(() => Object.values(selectedForBulk).filter(Boolean).length, [selectedForBulk]);
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3">
              <button
                type="button"
                onClick={() => beginBulkPayForSelected(Object.keys(selectedForBulk).filter((id) => selectedForBulk[id]))}
                disabled={selectedCount === 0}
                className={`rounded-full bg-indigo-600 px-4 py-2 text-sm font-semibold text-white ${selectedCount === 0 ? 'opacity-50 pointer-events-none' : 'hover:bg-indigo-500'}`}
              >
                Bulk pay selected ({selectedCount})
              </button>
                            ...
                            <div key={item.id} className="rounded-2xl border border-slate-200 bg-white p-3">
                              <div className="flex items-start justify-between gap-3">
                                <div className="flex items-center gap-3">
                                  <input
                                    type="checkbox"
                                    checked={!!selectedForBulk[item.id]}
                                    onChange={() => setSelectedForBulk((s) => ({ ...s, [item.id]: !s[item.id] }))}
